...
import { LayoutDashboard, ScanSearch, Trophy, ShieldAlert } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-slate-300 font-sans overflow-hidden select-none">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <header className="h-20 px-8 border-b border-slate-800/60 flex items-center justify-between shrink-0 bg-[#020617]/80 backdrop-blur-md z-10">
          ...
          <div className="flex items-center gap-4">
            {address ? (
              ...
              <button 
                onClick={connect}
                className="px-5 py-2.5 bg-white text-black rounded-full text-sm font-bold hover:bg-slate-200 transition-transform active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.1)]"
              >
                Connect Wallet
              </button>
            )}
        ...
        {/* Bottom Action Bar */}
        <div className="absolute bottom-0 left-0 right-0 h-1 border-t-0 bg-gradient-to-r from-transparent via-indigo-500/20 to-transparent"></div>
      </main>
    </div>
  );
}
