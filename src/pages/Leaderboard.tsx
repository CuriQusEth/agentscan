...
export default function Leaderboard({ onAgentClick }: { onAgentClick?: (id: string) => void }) {
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      {topAgents.length > 0 && (
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end mb-12">
           {/* Rank 2 */}
           {topAgents[1] && (
             <motion.div onClick={() => handleAgentClick(topAgents[1].registryId)} style={{ cursor: topAgents[1].registryId ? 'pointer' : 'default' }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-slate-900/40 p-6 rounded-t-3xl border border-slate-800/60 border-b-0 h-[280px] flex flex-col justify-end text-center relative shadow-[0_0_30px_rgba(255,255,255,0.02)] hover:bg-slate-800/50 transition-colors">
                ...
                <div className="text-3xl font-bold text-emerald-400 flex items-center justify-center gap-2">
                  {topAgents[1].trustScore} <span className="text-sm font-medium text-slate-500">score</span>
                </div>
             </motion.div>
           )}
           ...
           {/* Rank 1 */}
           {topAgents[0] && (
             <motion.div onClick={() => handleAgentClick(topAgents[0].registryId)} style={{ cursor: topAgents[0].registryId ? 'pointer' : 'default' }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gradient-to-t from-indigo-900/20 to-slate-900/60 p-6 rounded-t-3xl border border-indigo-500/30 border-b-0 h-[320px] flex flex-col justify-end text-center relative shadow-[0_-10px_40px_rgba(79,70,229,0.1)] z-10 hover:from-indigo-800/30 hover:to-slate-800/60 transition-colors">
                ...
                <div className="text-4xl font-bold text-white flex items-center justify-center gap-2 drop-shadow-md">
                   {topAgents[0].trustScore} <span className="text-sm font-medium text-indigo-300">score</span>
                </div>
           ...
           {/* Rank 3 */}
           {topAgents[2] && (
             <motion.div onClick={() => handleAgentClick(topAgents[2].registryId)} style={{ cursor: topAgents[2].registryId ? 'pointer' : 'default' }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-slate-900/40 p-6 rounded-t-3xl border border-slate-800/60 border-b-0 h-[260px] flex flex-col justify-end text-center relative shadow-[0_0_30px_rgba(255,255,255,0.02)] hover:bg-slate-800/50 transition-colors">
                ...
                <div className="text-2xl font-bold text-emerald-400 flex items-center justify-center gap-2">
                  {topAgents[2].trustScore} <span className="text-sm font-medium text-slate-500">score</span>
                </div>
             </motion.div>
           )}
         </div>
        ...
        <div className="bg-slate-900/40 rounded-3xl border border-slate-800/60 p-2">
          <div className="divide-y divide-slate-800/40">
            {restAgents.map((agent, i) => (
              <div key={agent.id} onClick={() => handleAgentClick(agent.registryId)} className={`flex items-center gap-6 p-4 hover:bg-slate-800/20 transition-colors rounded-2xl ${agent.registryId ? 'cursor-pointer' : ''}`}>
                ...
                <div className="shrink-0 flex items-center gap-2">
                   <ShieldCheck size={16} className={agent.trustScore > 75 ? 'text-emerald-400' : 'text-amber-400'} />
                   <span className="font-bold text-lg text-white">{agent.trustScore}</span>
                </div>
              </div>
            ))}
          </div>
